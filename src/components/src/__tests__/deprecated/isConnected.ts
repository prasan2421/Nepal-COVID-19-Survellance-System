  ...
  describe('isInternetReachable', () => {
    describe('fetch', () => {
      it('should resolve to true when the native module returns a connected state', () => {
        ...
        return expect(NetInfo.isInternetReachable.fetch()).resolves.toBe(true);
      });
      ...
      it('should resolve to true when the native module returns a not connected state', () => {
        ...
        return expect(NetInfo.isInternetReachable.fetch()).resolves.toBe(false);
      });
    });
    ...
    describe('Event listener callbacks', () => {
      it('should call the listener when listening even if no event is emitted', () => {
        ...
        NetInfo.isInternetReachable.addEventListener('connectionChange', listener);
      ...
      it('should call all listeners when the native event is emmitted', () => {
        ...
        NetInfo.isInternetReachable.addEventListener('connectionChange', listener1);
        NetInfo.isInternetReachable.addEventListener('connectionChange', listener2);
      ...
      it('should not call the listener after being removed', () => {
        const listener = jest.fn();
        NetInfo.isInternetReachable.addEventListener('connectionChange', listener);
        NetInfo.isInternetReachable.removeEventListener('connectionChange', listener);
      ...
      it('should call the remaining listeners when one has been removed', () => {
        ...
        NetInfo.isInternetReachable.removeEventListener('connectionChange', listener1);
