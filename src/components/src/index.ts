...
const _isInternetReachableListeners = new Map<
  DeprecatedTypes.isInternetReachableHandler,
...
export const isInternetReachable = {
  /**
   * Deprecated method to listen for changes to the connected boolean. You should now use the
   * normal `addEventListener` method and look at the `isInternetReachable` property.
   *
   * @deprecated
   */
  addEventListener: (
    eventName: string,
    handler: DeprecatedTypes.isInternetReachableHandler,
  ): DeprecatedTypes.Subscription => {
    ...
    const listener = (state: Types.NetInfoState): void => {
      handler(DeprecatedUtils.isInternetReachable(state));
    };

    _isInternetReachableListeners.set(handler, listener);
    State.add(listener);
   ...
   * the normal `addEventListener` method and look at the `isInternetReachable` property.
   *
   * @deprecated
   */
  removeEventListener: (
    ...
    const listener = _isInternetReachableListeners.get(handler);
    listener && State.remove(listener);
    _isInternetReachableListeners.delete(handler);
  },

  /**
   * Deprecated method to get the current is connected boolean. You should now use the normal
   * `fetch` method and look at the `isInternetReachable` property.
   *
   * @deprecated
   */
  fetch: (): Promise<boolean> => {
    return State.latest().then(DeprecatedUtils.isInternetReachable);
  },
};
