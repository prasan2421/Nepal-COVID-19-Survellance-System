...
export default class FAQsScreen extends Component {
    ...
    componentDidMount() {
        ...
        SessionHelper.getFAQsHtml().then(faqsHtml => {
            let html = '';
    ...
    checkLanguage= async () =>{
        ...
        if (language){
            this.props.navigation.setParams({ Count: language=='en'?'FAQs':'धेरै सोधिएका प्रश्नहरू' });
            this.setState({
                language: language
            })
        }
    ...
    loadFAQs = () => {
        ...
        let url=Url.baseUrl+'contents/faqs'
        fetch(url, {
            headers: {
