...
class New extends React.Component<{}, State>  {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            data:[
                ...
                {id:3,name:'Do you have exchaustion?',checked:false,value1:3.8,value2:0.5},
    ...
    saveFeedback = async() => {
        if((this.state.data.filter(cat=>cat.checked===true)).length===0  && !this.state.travelled ) {
            Alert.alert('Alert','Please select at lease one symptom.')
            return;
        }
            ...
            .then((response)=> {
                if(!response.ok) {
                    ...
                    response.json()
                        .then ((responseJson) => {
                            if(responseJson.success){
                                this.props.navigation.navigate('UpdatedResultEnglish',{
