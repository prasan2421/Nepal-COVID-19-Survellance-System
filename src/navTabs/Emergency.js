...
class New extends React.Component<{}, State> {
    ...
    next = async() =>  {
        ...
        if(note==""){
            note_Error = 'कृपया नोट लेख्नु होस्';
        }
        ...
        if(name_Error ||  number_Error || note_Error || pin_Error) {
            return;
        }
        ...
        let url=Url.baseUrl+'client-messages'
        fetch(url,{
            method:'POST',
            headers:{
                ...
                if(!response.ok) {
                    ...
                    response.json()
                        .then ((responseJson) => {
                            if(responseJson.success){
// this.hideLoader();
                                this.props.navigation.navigate('SuccessEmergency');
        ...
        if(this.state.dataName.length>0){
            return(
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    ...
                    <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                        <View style={{width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                            <TextInput style={{paddingLeft:10,width:'100%',height:150}}
                                       placeholder="तपाईलाई अत्यावश्यक परेको अवस्थामा मात्र आपतकालीन नोटमा क्लीक गरेर सन्देश छोड्नुहोस् | हाम्रा चिकित्सकहरुले तपाईको जिज्ञासा निराकरण गर्नुहुनेछ |"
                                       returnKeyType='done'
                                       multiline={true}
                                       onChangeText={(note) => this.setState({note})}
                            />
                        </View>
                        <ErrorText text={this.state.note_Error} />
                    </View>
                </View>
            )
        }
