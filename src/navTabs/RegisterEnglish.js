...
class New extends React.Component<{}, State> {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            diseases:[
                {id:0,name:'Cancer',checked:false},
                {id:1,name:'Diabetes',checked:false},
                {id:2,name:'Blood pressure',checked:false},
                {id:3,name:'Lung diseases',checked:false},
                {id:4,name:'Liver diseases',checked:false},
                {id:5,name:'Others',checked:false},
            ],
            ifDiseases:false,
            count:[],
...
// alert(JSON.stringify(finalCountries.join(',')));return;
        ...
        if(sex==""){
            sex_Error = 'Please select your sex.';
        }
        if(number.length<10){
            number_Error = 'Length of phone no. is not enough';
        }
        if(number==""){
            number_Error = 'Please enter your mobile no.';
        }
        if(secondary_number==""){
            secondary_number_Error = 'Please enter the secondary number';
        }
        if(province==""){
            province_Error = 'Please select your province.';
        }
        if(district==""){
            district_Error = 'Please select your district.';
        }
        if(municipality==""){
            municipality_Error = 'Please select your municipality.';
        }

        if(ward==""){
            ward_Error = 'Please enter your ward no.';
        }
        if(nationality==""){
            nationality_Error = 'Please enter your nationality.';
        }
        if(pin==""){
            pin_Error = 'Please enter the pin no.';
        }
        if(contact==""){
            contact_Error = 'Please enter the number of person in contact.';
        }
        if(health==""){
        ...
        if(location==""){
            locationError = 'Please mark your location.';
        }
        ...
        if(name_Error || age_Error || sex_Error ||  number_Error || pin_Error || province_Error || district_Error || municipality_Error ||  ward_Error || contact_Error ||  locationError || nationality_Error) {
            return;
        }
        this.showLoader();
                ...
                if(!response.ok) {
                    ...
                    response.json()
                        .then ((responseJson) => {
                            if(responseJson.success){
// this.hideLoader();
                                this.props.navigation.navigate('SuccessRegisteredEnglish',{
                                    name:pin,
                                    number:number
                                });
            ...
            return(
                <View style={{marginBottom:10,elevation:2,position:'relative',width:'95%',flexDirection: 'row',alignItems:'center',justifyContent:'center',backgroundColor:'#eae98f',padding:10,borderRadius:5}}>
                    <View style={{paddingVertical:4,paddingHorizontal:15,borderRadius:5}}>
                        <Text style={{color:'#333333',fontSize:14}}><Text style={{fontWeight:'bold'}}>Pin no.</Text>: {this.state.pinNumber}</Text>
                    </View>
                </View>
            )
        }
            ...
            >
                        ...
                        <TouchableWithoutFeedback onPress={()=>''}>
                            <View
                                ...
                                <ScrollView
                                    ...
                                    <Text style={{fontWeight:'bold',marginBottom:10}}>Select long term diseases :</Text>
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{flexDirection:'row',height:48,alignItems:'center',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                        ...
                        {/*           onChangeText={(gps) => this.setState({gps})}*/}
                        {/*/>*/}
                        <Text style={{paddingLeft:10,width:'100%',color:'#b1b1b1'}}>Province : 3</Text>
                        {/*<Picker*/}
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{flexDirection:'row',height:48,alignItems:'center',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1,marginVertical:0}}>
                        ...
                        {/*           onChangeText={(gps) => this.setState({gps})}*/}
                        {/*/>*/}
                        <Text style={{paddingLeft:10,width:'100%',color:'#b1b1b1'}}>District : Kathmandu</Text>
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{flexDirection:'row',height:48,alignItems:'center',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                        ...
                        {/*           onChangeText={(gps) => this.setState({gps})}*/}
                        {/*/>*/}
                        <Text style={{paddingLeft:10,width:'100%',color:'#b1b1b1'}}>Municipality : Kathmandu</Text>
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{flexDirection:'row',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                        <Picker
                            ...
                            <Picker.Item label="Select a ward no." value="" />
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View
                        ...
                        style={{flexDirection:'row',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                        <TextInput style={{paddingLeft:10,width:'100%'}}
                                   placeholder="Number of people in close contact recently"
                                   returnKeyType='done'
                                   keyboardType={'numeric'}
                                   onSubmitEditing={Keyboard.dismiss}
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{flexDirection:'row',justifyContent:'space-between',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                        <Picker
                            ...
                            <Picker.Item label="Do you have any long term diseases ?" value="" />
