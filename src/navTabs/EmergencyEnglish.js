...
class New extends React.Component<{}, State> {
    ...
    getNamesFromAsyncStorage = async () => {
        ...
        if(number==""){
            number_Error = 'Please enter a phone no.';
        }

        if(pin==""){
            pin_Error = 'Please enter a pin no.';
        }
    ...
    next = async() =>  {
        ...
        if(name==""){
            name_Error = 'Please select a name';
        }
                    ...
                    <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                        <View style={{flexDirection:'row',width:'95%',backgroundColor:'#fff',borderRadius:5,elevation: 1}}>
                            <TextInput style={{paddingLeft:10,width:'100%',height:150}}
                                       placeholder="Click on the emergency note and send only when you need it most. Physicians will solve your curiosity"
                                       returnKeyType='done'
                                       multiline={true}
